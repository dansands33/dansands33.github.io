...
No application dependencies, npm install, or build step. Serve the folder over
HTTP so the crest files can load; the site itself is plain HTML, CSS, and JS.
...
│       └── aurelius.css
├── js/
│   ├── content.js          # i18n copy and required ASCII crest text-file loader
...
> **Note:** `css/themes/*.txt` are the source of truth for the hero crests. The
> browser loads these files at startup; there is no inline art fallback. The text
> assets are generated by `scripts/ascii_crest.py`.
...
|-------|---------|----------------------|------|
| **sunset** | techy orange ⇄ cyan | Constellation network — drifting nodes that connect when close; parallax on mouse | cyberpunk dusk |
...
| **aurelius** *(default)* | black · gold · silver | "Chapters being written" — gold sentences **rain in staggered** (random per-column spawn) like slow rain; a unique black+gold bust crest | classical, editorial |
...
The site has no application server or backend requirement. It does use `fetch()`
to load the theme crest text files, and browsers block local `file://` fetches;
serve the folder with any static HTTP server (Python below is one option):
...
This static server only makes files available over HTTP. It does not build the
site, modify files, or provide application logic. The page waits for all theme
crest files and reports an error if any required file cannot be loaded.
